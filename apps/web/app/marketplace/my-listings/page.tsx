      ...
      {listings.length === 0 ? (
        ...
        <div className="space-y-4">
          {listings.map((listing) => {
            ...
            return (
              <div key={listing.id} className="bg-card border border-border rounded-xl overflow-hidden">
                <div className="p-4 flex gap-4 items-start">
                  {card.imageSmall ? (
                    <div className="w-14 h-20 relative shrink-0 bg-black/20 rounded-lg overflow-hidden">
                      <Image src={card.imageSmall} alt={card.name} fill className="object-contain p-1" unoptimized />
                    </div>
                  ) : (
                    <div className="w-14 h-20 shrink-0 bg-black/20 rounded-lg flex items-center justify-center text-muted-foreground text-xs">
                      No img
                    </div>
                  )}
                      ...
                      <div key={offer.id} className="px-4 py-3 flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <Badge variant={OFFER_STATUS_COLOR[offer.status] ?? "outline"} className="text-xs">
                              {offer.status}
                            </Badge>
                          </div>
                          <div className="text-xs text-muted-foreground space-y-0.5">
                            <p className="font-medium">{OFFER_TYPE_LABELS[offer.offerType] ?? offer.offerType}</p>
