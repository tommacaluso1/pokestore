    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        ...
        <div className="aspect-square relative bg-card border border-border rounded-xl overflow-hidden">
          {card.imageSmall ? (
            <Image src={card.imageLarge ?? card.imageSmall} alt={card.name} fill className="object-contain p-6" unoptimized />
          ) : (
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-0.5">{card.tcgSet.name}</p>
              </div>
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {listing.quantity > 1 && (
              <Badge variant="outline" className="text-xs">Qty: {listing.quantity}</Badge>
            )}
          </div>
          ...
          <div className="pt-3 border-t border-border">
            ...
            <p className="text-sm font-medium mt-0.5">{listing.seller.name ?? listing.seller.email}</p>
            ...
            <div key={offer.id} className="bg-card border border-border rounded-xl p-4 space-y-2">
              ...
              <div className="text-sm text-muted-foreground space-y-0.5">
                ...
                {offer.items.length > 0 && (
                  <p>Cards: {offer.items.map((i) => `${i.userCard.card.name} ×${i.quantity}`).join(", ")}</p>
                )}
